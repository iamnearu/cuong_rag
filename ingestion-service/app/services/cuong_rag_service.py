...
Orchestrator for the CuongRAG pipeline:
    Document → MinerU/Docling Parse → Local Vector Index + LightRAG KG → Hybrid Retrieval
...
class CuongRAGService:
    """
    Full CuongRAG pipeline orchestrator.

    Phases:
    1. PARSING  — MinerU/Docling parse → markdown + chunks + images
    2. INDEXING — Embed chunks → PostgreSQL pgvector + ingest markdown → LightRAG KG
    ...
    def __init__(
        ...
        if settings.CUONGRAG_ENABLE_KG:
    ...
    async def process_document(self, document_id: int, file_path: str) -> int:
        """
        Process a document through the full CuongRAG pipeline.
        ...
        try:
            ...
            if parsed.chunks:
                def _index_sync():
                    # Embed and store in PostgreSQL pgvector
            ...
            logger.error(f"CuongRAG failed for document {document_id}: {e}")
    ...
    async def delete_document(self, document_id: int) -> None:
        ...
        logger.info(f"Deleted document {document_id} from CuongRAG stores")
