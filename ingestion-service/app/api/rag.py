...
async def query_documents(
    ...
    from app.services.cuong_rag_service import CuongRAGService
    if isinstance(rag_service, CuongRAGService) and request.mode != "vector_only":
...
async def reindex_document(
    ...
    """Re-process an existing document through the CuongRAG pipeline."""
    result = await db.execute(select(Document).where(Document.id == document_id))
    document = result.scalar_one_or_none()
    ...
    try:
        ...
        return DocumentProcessResponse(
            ...
            message=f"Re-indexed with CuongRAG: {chunk_count} chunks created"
        )
    except Exception as e:
        raise HTTPException(
...
async def reindex_workspace(
    ...
    and re-processes every document through the CuongRAG pipeline.
...
async def get_workspace_rag_stats(
    ...
    # Count CuongRAG documents (parser_version = 'docling' / 'deepseek_ocr' / 'mineru')
    ...
    return ProjectRAGStatsResponse(
        ...
        cuongrag_documents=cuongrag_documents,
    )
...
async def _get_kg_service(workspace_id: int):
    """Get KnowledgeGraphService for a knowledge base (if CuongRAG is active)."""
...
async def get_workspace_analytics(
    ...
    # KG analytics (optional — only if CuongRAG active)
    kg_analytics = None
    if cuongrag_documents > 0:
        try:
            kg = await _get_kg_service(workspace_id)
...
# ---------------------------------------------------------------------------
# Chat endpoint — LLM-powered document Q&A via CuongRAG
# ---------------------------------------------------------------------------
...
async def chat_with_documents(
    ...
    """Chat with documents using CuongRAG retrieval + LLM answer generation."""
    kb = await verify_workspace_access(workspace_id, db)

    rag_service = get_rag_service(db, workspace_id)

    # -- 1. Retrieve relevant chunks via CuongRAG --
    chunks = []
    citations = []
    kg_summary = ""

    from app.services.cuong_rag_service import CuongRAGService
    if isinstance(rag_service, CuongRAGService):
        result = await rag_service.query_deep(
