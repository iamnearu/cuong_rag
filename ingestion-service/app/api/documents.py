...
def _markdown_page_to_blocks(page_markdown: str, workspace_id: int) -> list[dict]:
    blocks: list[dict] = []
    ...
    while i < len(lines):
        ...
        if im:
            flush_paragraph()
            image_url = im.group(2).strip()
            if image_url.startswith("/static/"):
                resolved_url = image_url
            else:
                resolved_url = f"/static/doc-images/kb_{workspace_id}/{image_url.lstrip('./')}"
            blocks.append({"type": "image", "image_url": resolved_url, "caption": im.group(1).strip()})
            i += 1
            continue
        ...
        if "<table" in line.lower():
            ...
            rows = _parse_html_table_rows(line)
            blocks.append({"type": "table", "table_id": table_id, "rows": rows})
            i += 1
            continue
        ...
        if line.startswith("|"):
            ...
            rows: list[list[str]] = []
            for k, tline in enumerate(table_lines):
                if k == 1 and re.search(r"\|\s*:?-{3,}", tline):
                    continue
                cells = [c.strip() for c in tline.split("|") if c.strip()]
                if cells:
                    rows.append(cells)

            blocks.append({"type": "table", "table_id": str(uuid.uuid4()), "rows": rows})
            i = j
            continue
...
def _markdown_to_ocr_document(markdown: str, source_filename: str, parser_version: str | None, workspace_id: int) -> OCRDocumentBody:
    pages_raw = [p for p in re.split(r"\n\s*---\s*\n", markdown) if p.strip()]
    if not pages_raw:
        pages_raw = [markdown]
    ...
    pages = [
        OCRContentPage(
            page_number=i + 1,
            blocks=_markdown_page_to_blocks(page_md, workspace_id),
        )
        for i, page_md in enumerate(pages_raw)
    ]

    return OCRDocumentBody(
...
async def get_document_markdown(
    ...
    """Get the full structured markdown content of a document (CuongRAG parsed)."""
    result = await db.execute(select(Document).where(Document.id == document_id))
    document = result.scalar_one_or_none()
    ...
    if not document.markdown_content:
        raise HTTPException(
            ...
            detail="No markdown content available. Document may not have been processed with CuongRAG."
        )
...
@router.get("/{document_id}/ocr-structured", response_model=DocumentOCRStructuredResponse)
async def get_document_ocr_structured(
    document_id: int,
    db: AsyncSession = Depends(get_db),
):
    """Get OCR structured output schema (MinerU-like) derived from stored markdown."""
    result = await db.execute(select(Document).where(Document.id == document_id))
    document = result.scalar_one_or_none()
    ...
    if not document.markdown_content:
        return DocumentOCRStructuredResponse(
            document_id=document_id,
            status=document.status,
            num_pages=document.page_count or 0,
            markdown_available=False,
            document=None,
        )

    ocr_doc = _markdown_to_ocr_document(
        markdown=document.markdown_content,
    ...
    return DocumentOCRStructuredResponse(
        document_id=document_id,
        status=document.status,
        num_pages=ocr_doc.metadata.total_pages,
