"""
CuongRAG Phase 3 — RAGAS Synthetic Testset Generation + Evaluation.
...
Workflow:
  ...
  3. Run each question through the CuongRAG chat endpoint
...
BASE_URL = os.environ.get("RAG_API_BASE_URL", "http://localhost:8081/api/v1/rag")
TIMEOUT = 120
...
# ── Step 3: Run testset through CuongRAG chat ─────────────────────────────────
...
def run_testset_through_chat(
    ...
    print(f"\n[3/4] Running {len(samples)} questions through CuongRAG chat...")
...
def main():
    parser = argparse.ArgumentParser(
        description="CuongRAG Phase 3 — RAGAS Synthetic Testset Generation + Evaluation"
    )
